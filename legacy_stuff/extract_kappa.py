...
datasets =["NeuroLF_Hoffman_Dataset", "Siemens_mMR_NEMA_IQ", "Siemens_Vision600_thorax", "Siemens_mMR_ACR", "Mediso_NEMA_IQ"]
...
for dataset in datasets:
    print("Look at: ", dataset)
    srcdir = Path("/mnt/share/petric" + "/" + dataset)

    #OSEM_image = STIR.ImageData(str(srcdir / 'OSEM_image.hv'))
    #GT_image = STIR.ImageData(str(srcdir / 'PETRIC' /  'reference_image.hv'))
    kappa_image = STIR.ImageData(str(srcdir /  'kappa.hv'))

    print("Norm of kappa: ", kappa_image.norm() / np.prod(kappa_image.shape))
