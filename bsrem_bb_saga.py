    ...
    def update(self):
        if self.iteration == 0: 
            ...
            for i in range(self.num_subsets):
                gm = self.subset_gradient_pll(self.x, self.subset_order[i]) 
                self.gm[self.subset_order[i]] = gm - prior_grad
                g.add(gm, out=g)
            
            gradient = g / self.num_subsets - prior_grad
            self.sum_gm = g - prior_grad * self.num_subsets

            if prior_grad.norm()/gradient.norm() > 0.5:
        ...
        if self.compute_rdp_diag_hess:
            print(self.iteration % self.num_subsets)
            if self.iteration % self.num_subsets == 0 and self.iteration > 0:
                print("Compute Precond Again!")
                self.rdp_diag_hess_obj.compute(self.x, self.precond)
            gradient.divide(self.lkhd_precond + self.precond + self.eps, out=self.x_update)
        else:
        ...
        if self.iteration == 0:
            ...
            self.c = self.c ** ((k-2)/(k-1)) * (step_size*phik) ** (1/(k-1))
